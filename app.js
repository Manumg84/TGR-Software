...
function renderClientsTable(searchTerm = '', filterBy = 'nameOrCompany') {
    ...
    chatMessageInput.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') {
         ...
         // === INICIO DE NUEVA LÓGICA DE CELDAS EDITABLES ===
    function makeTableEditable(tableId, excludeCols = []) {
        ...
        Array.from(tbody.rows).forEach(row => {
            Array.from(row.cells).forEach((cell, idx) => {
                // Excluir columnas con select, input (menos texto) o botones
                if (excludeCols.includes(idx)) return;
                // Solo para celdas con input de texto o número
                const input = cell.querySelector('input[type="text"], input[type="number"]');
                if (!input) return;

                cell.classList.add('table-editable-cell');
                ...
                cell.addEventListener('dblclick', function () {
                    ...
                    input.readOnly = false;
                    input.focus();
                    // Quitar el fondo amarillo cuando termina
                    ...
                    function finishEdit() {
                        // Guardar valor nuevo (ya está en el input.value)
                        input.readOnly = true;
                        cell.classList.remove('editing');
                        // Tooltip si largo
                        if (input.value.length > 18 || cell.scrollWidth > cell.clientWidth) {
                            cell.setAttribute('title', input.value);
                        } else {
                ...
                // Tooltip: mostrar si largo
                cell.addEventListener('mouseenter', function () {
                    const value = input.value;
                    setTimeout(() => {
                        if ((value.length > 18 || cell.scrollWidth > cell.clientWidth) && !cell.classList.contains('editing')) {
    ...
    function hookEditableLines() {
        setTimeout(() => {
            // workHoursTable: Operario, Categoría, Tipo, Fecha, Concepto, Horas, Importe, [botón]
            // Excluir 0,1,2(select), 6(importe, readonly), 7(botón), solo editar fecha, concepto, horas
            makeTableEditable('workHoursTable', [0,1,2,6,7]);

            // materialsTable: Proveedor(select), Albarán, Fecha, Concepto, Cant., €/ud, Dto %, € Dto, Total, [botón]
            // Excluir 0(select), 7(descuento, readonly), 8(total, readonly), 9(botón)
            makeTableEditable('materialsTable', [0,7,8,9]);
        }, 250);
    }

    if (typeof ordenTrabajoModalEl !== "undefined") {
        ordenTrabajoModalEl.addEventListener('shown.bs.modal', hookEditableLines);
    }

    const origAddWorkHourRow = typeof addWorkHourRow === "function" ? addWorkHourRow : null;
    const origAddMaterialRow = typeof addMaterialRow === "function" ? addMaterialRow : null;
    if (origAddWorkHourRow) {
        window.addWorkHourRow = function (...args) {
            const res = origAddWorkHourRow.apply(this, args);
            hookEditableLines();
            return res;
        }
    }
    if (origAddMaterialRow) {
        window.addMaterialRow = function (...args) {
            const res = origAddMaterialRow.apply(this, args);
            hookEditableLines();
            return res;
        }
    }
    // === FIN DE NUEVA LÓGICA DE CELDAS EDITABLES ===
